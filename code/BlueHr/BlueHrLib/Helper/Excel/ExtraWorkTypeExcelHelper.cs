                    ...
                    if (ep.Workbook.Worksheets.Count > 0)
                    {
                        ...
                        for (int i = 2; i <= ws.Dimension.End.Row; i++)
                        {
                            records.Add(new ExtraWorkRecordExcelModel()
                            {
                                ...
                                ExtraWorkTypeStr = ws.Cells[i, 3].Value == null ? string.Empty : ws.Cells[i, 3].Value.ToString(),
                                Duration = ws.Cells[i, 4].Value == null ? string.Empty : ws.Cells[i, 4].Value.ToString(),
                                DurationTypeStr = ws.Cells[i, 5].Value == null ? string.Empty : ws.Cells[i, 5].Value.ToString(),
                                OtReason = ws.Cells[i, 6].Value == null ? string.Empty : ws.Cells[i, 6].Value.ToString() 
                            });
                        }
                    }
                    else
                        ...
                        if (records.Where(s => s.ValidateMessage.Success == false).Count() > 0)
                        {
                            ...
                            using (ExcelPackage ep = new ExcelPackage(tmpFileInfo))
                            {
                                ...
                                for (int i = 0; i < records.Count(); i++)
                                {
                                    ...
                                    sheet.Cells[i + 2, 3].Value = records[i].ExtraWorkTypeStr;
                                    sheet.Cells[i + 2, 4].Value = records[i].Duration;
                                    sheet.Cells[i + 2, 5].Value = records[i].DurationTypeStr;
                                    sheet.Cells[i + 2, 6].Value = records[i].OtReason; 
